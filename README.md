...
| 📦 **多开隔离启动**           | 特色功能。直接启动系统已注册的 Codex 客户端，并为每个账号分别隔离 `CodexHome`、`APPDATA`、`LOCALAPPDATA` 和 `--user-data-dir`。                 |
...
- **多开隔离模式**：特色模式。直接启动系统已注册的 Codex 客户端，并为每个账号使用独立配置和运行环境。

多开隔离模式会隔离 `CodexHome`、`APPDATA`、`LOCALAPPDATA` 和浏览器 `--user-data-dir`，不再复制 WindowsApps 中的客户端程序：
...
- **启动模式**：可在账号切换模式和多开隔离模式之间切换；多开账号直接使用系统已注册的客户端程序。
