  ...
  return (
    <div className="grid gap-4">
      <div className="grid grid-cols-4 gap-3 max-[1120px]:grid-cols-2 max-[720px]:grid-cols-1">
        <Metric title="账号总数" value={profiles.length || appState.profileCount} note="已保存账号" />
        <Metric title="运行中" value={appState.runningCount} note="当前 Codex 实例" statusText={appState.runningCount > 0 ? "活跃" : "空闲"} />
        <Metric title="健康账号" value={healthyCount} note="资料目录和认证文件正常" statusText="正常" />
        <Metric
          title="异常账号"
          value={abnormalCount}
          note="缺少目录或 auth.json"
          statusText={abnormalCount > 0 ? "需处理" : "正常"}
      ...
      <div className="grid grid-cols-2 gap-4 max-[960px]:grid-cols-1">
        <section className={sectionClass}>
          <SectionTitle icon={<Settings2 size={16} />} title="当前模式" />
          <div className="mt-4 grid gap-3">
            <InfoRow label="启动模式" value={formatLaunchMode(appState.launchMode)} />
            <InfoRow label="配置模式" value={appState.launchMode === "multi" ? "账号 CodexHome 配置" : "系统配置"} />
            <InfoRow label="当前账号" value={appState.activeProfile || "未选择"} />
            <InfoRow label="Codex 命令" value={appState.codexCommandAvailable ? "可用" : "不可用"} tone={appState.codexCommandAvailable ? "green" : "amber"} />
          </div>
        </section>

        <section className={sectionClass}>
          <SectionTitle icon={<Folder size={16} />} title="当前设置" />
          <div className="mt-4 grid gap-3">
            <PathRow label="账号资料根目录" value={appState.profileRoot || "未设置"} ok={appState.profileRootExists} />
            <PathRow label="auth.json" value={appState.activeAuthPath || "未找到"} ok={appState.activeAuthExists} />
            <PathRow label="config.toml" value={appState.activeConfigPath || "未找到"} ok={appState.activeConfigExists} />
          </div>
        </section>
      </div>
      ...
      <section className={sectionClass}>
        <div className="flex items-center justify-between gap-4 border-b border-[#ecf1f6] pb-4">
          <SectionTitle icon={<CheckCircle2 size={16} />} title="账号状态" />
              ...
              <div key={profile.name} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {profile.active ? <StatusPill tone="blue" text="当前账号" /> : null}
                    <StatusPill tone={profile.running ? "green" : "slate"} text={profile.running ? "运行中" : "就绪"} />
                    {!profile.authExists ? <StatusPill tone="amber" text="认证缺失" /> : null}
                  </div>
  ...
  return (
    <div className="grid gap-1 rounded-[8px] bg-slate-50 px-3 py-2.5">
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm font-semibold text-slate-500">{label}</span>
        <StatusPill tone={ok ? "green" : "amber"} text={ok ? "存在" : "缺失"} />
      </div>
...
function formatLaunchMode(mode: AppState["launchMode"]) {
  return mode === "multi" ? "多开隔离模式" : "账号切换模式";
}
